...
{-# OPTIONS_GHC -fwarn-incomplete-patterns -fno-warn-unticked-promoted-constructors #-}
-- CIS 194 Spring 2015: Homework 8
...
transposition = Conj dir1 dir2
    where
      ...
      dir1 = 
          modus_tollens
      -- Case 2: (~Q -> ~P) -> (P -> Q)
      dir2 not_q_imp_not_p p = 
          case excluded_middle of
            -- SCase 1: Q, obvious
            Left  q     -> q
            -- SCase 2: ~Q, then ~P since ~Q -> ~P
            -- This is a contradiction since we have both P and ~P
            Right not_q -> absurd $ not_q_imp_not_p not_q p
...
demorgan = Conj dir1 dir2
    where
      -- Case 1: ~(P \/ Q) -> (~P /\ ~Q)
      dir1 not_pq = 
          Conj (not_pq . Left) (not_pq . Right)
      -- Case 2: (~P /\ ~Q) -> ~(P \/ Q)
      dir2 (Conj not_p _) (Left  p) = 
          not_p p
      dir2 (Conj _ not_q) (Right q) = 
          not_q q
